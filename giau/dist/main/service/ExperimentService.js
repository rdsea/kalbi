...
class ExperimentService {
    constructor(repository, topologyService, expResultService, depPatternService, depPatternMatcher, logger) {
        this.repository = repository;
        this.topologyService = topologyService;
        this.expResultService = expResultService;
        this.depPatternService = depPatternService;
        this.depPatternMatcher = depPatternMatcher;
        this.logger = logger;
    }
    /**
    ...
    readExperimentsByDeploymentPattern(node) {
        return __awaiter(this, void 0, void 0, function* () {
            DeploymentPatternValidation_1.DeploymentPatternValidation.validatePureNode(node);
            let mostSimilarDeploymentPattern = yield this.depPatternMatcher.findSimilarDeploymentPattern(node);
            if (!mostSimilarDeploymentPattern) {
                this.logger.info('Deployment pattern cant be found');
                return [];
            }
            try {
                let expsDataModel = yield this.repository.findAllForDeploymentPattern(mostSimilarDeploymentPattern._id);
                let experiments = [];
                for (let expDataModel of expsDataModel) {
                    let exp = yield this.readOne(expDataModel._id.toString());
                    if (exp) {
                ...
                if (e instanceof PersistenceException_1.PersistenceException) {
                    throw new ServiceException_1.ServiceException('Experiments cannot be fetched from DB');
                }
                else {
                    throw e;
                }
